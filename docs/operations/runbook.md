...
- OPENAI_API_KEY / OPENAI_MODEL: optional grounded agent-explanation provider configuration.
...
- APP_ACCESS_TOKEN: random pilot token of at least 32 characters. Production
  online agent/chat, registry, uploads and backup require an access cookie or bearer token.
...
8 agent/chat requests per minute per route class, 100 combined AI requests per day,
and 60 requests per minute for each other route class. These are global pilot
limits, not per-user entitlements. Development uses bounded in-memory counters.
Provider retries are disabled; agent model time and output tokens are capped. The
deterministic agent path remains usable without model credentials, while budget
